#!/usr/bin/env bash
# Cloud-telemetry bridge installer — v2 (API uid lookup fix)
set -e
if [ ! -d bridge ]; then echo "ERROR: run from ~/print-queue (no bridge/ dir)"; exit 1; fi
echo "=== Installing cloud bridge v2 (API uid fix) ==="
...
echo ""
if grep -q "_fetch_uid" bridge/cloud_auth.py; then echo "VERIFY OK: new uid-lookup code is present."; else echo "VERIFY FAILED: old code still present!"; fi
